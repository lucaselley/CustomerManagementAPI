        ...
        private IEnumerable<AuditEntryEntity> CreateAuditEntities() {
            return this.ChangeTracker.Entries()
                .Where(entry => entry.Entity is IAuditable && entry.State switch {
                    ...
                    entityId: Guid.TryParse(entry.Properties.Single(property => property.Metadata.IsPrimaryKey())
                        ?.CurrentValue?.ToString(), out var id) ? id : Guid.Empty,
                    action: entry.State == EntityState.Added ? "INSERT" : entry.State == EntityState.Deleted ? "DELETE" : "UPDATE",
                    actor: this._httpContextAccessor.HttpContext.User.Claims.FirstOrDefault(claim =>
                        claim.Type.Equals(ClaimTypes.Name, StringComparison.OrdinalIgnoreCase))?.Value ?? "Unknown",
                    changes: entry.Properties.Select(property => new { property.Metadata.Name, property.CurrentValue })
